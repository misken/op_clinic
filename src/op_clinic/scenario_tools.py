...
def main(argv=None):
    ...
    create_configs_from_inputs_csv('exp11', './input/exp11_scenarios.csv', './input/config', '.')


def create_configs_from_inputs_csv(exp, scenarios_csv_file_path, config_path, run_script_path):
    """
    ...
    with open(run_script_file_path, 'w') as bat_file:
        ...
        for row in scenarios_df.iterrows():
            ...
            config_file_path = Path(config_path) / f'{exp}_scenario_{scenario}.cfg'

            with open(config_file_path, 'w', encoding='utf-8') as config_file:

                for index, value in scenario_row.items():
